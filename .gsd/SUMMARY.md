...
## 2026-07-14T15:36 — K-factor Elo-style Bounty System with One Piece Themed Ranks

Implementer: Created 3 new files, modified 10 existing files. Created `V6__update_bounty_default_and_backfill.sql` (+10) — Flyway migration changing bounty default to 1000, backfilling existing users, updating ranks. Created `BountyService.java` (+65) — K=32 Elo calculation with `updateBounties(winner, loser)` and static `computeRank(bounty, filiation)` method with 6-tier rank thresholds per filiation. Created `BountyServiceTest.java` (+148) — 9 unit tests covering equal ratings, underdog/favorite, bounty floor, rank promotion/demotion, pirate/marine threshold verification. Modified `AuthService.java` — changed starting bounty from 0 to 1000, derive initial rank via `BountyService.computeRank`. Modified `BoardService.java` — injected BountyService, call `updateBounties(attacker, loser)` after game-over win condition. Modified `GameService.java` — injected BountyService, call `updateBounties(winner, loser)` in IN_PROGRESS surrender branch. Modified `GameExpirationService.java` — injected BountyService, call `updateBounties(winner, loser)` in turn expiration handler. Modified `UserRepository.java` — replaced 4 formula-based JPQL queries with simple `ORDER BY u.bounty DESC` queries. Modified `LeaderboardService.java` — replaced `wins * winRate * 10000.0` formula with `(double) user.getBounty()`. Modified `client/src/app/page.tsx` — replaced 2 client-side `Math.round(entry.wins * entry.winRate * 10000)` with `entry.bounty`. Fixed 3 existing test files (`BoardServiceFireShotTest`, `GameServiceSurrenderTest`, `GameExpirationServiceTest`) to add `@Mock BountyService` for constructor injection.

Files changed: `V6__update_bounty_default_and_backfill.sql` (created, +10), `BountyService.java` (created, +65), `BountyServiceTest.java` (created, +148), `AuthService.java` (modified), `BoardService.java` (modified), `GameService.java` (modified), `GameExpirationService.java` (modified), `UserRepository.java` (modified), `LeaderboardService.java` (modified), `client/src/app/page.tsx` (modified), `BoardServiceFireShotTest.java` (modified), `GameServiceSurrenderTest.java` (modified), `GameExpirationServiceTest.java` (modified)
...
## 2026-07-15T11:12 — Frontend: Audio System (Sound Manager, Laugh/Scream/Soundtrack Triggers, Mute Setting)

Implementer: Created 2 new source files, modified 4 existing source files, created 21 placeholder MP3 files. Created `client/src/lib/sound/sound-context.tsx` (+148) — SoundProvider context with HTML5 Audio API, localStorage-persisted mute, useRef-based soundtrack management, autoplay handling via userInteractedRef + document click/touchstart/keydown listener, getAudioPath helper resolving avatar-specific or default audio. Created `client/src/lib/sound/index.ts` (+1) — barrel export. Modified `client/src/app/providers.tsx` (+4) — wrapped children with SoundProvider. Modified `client/src/app/game/[token]/battle-screen.tsx` (+6) — playLaugh on SUNK result in handleFire. Modified `client/src/app/game/[token]/page.tsx` (+40) — soundtrack lifecycle useEffect (start on IN_PROGRESS, stop on unmount), game-over audio useEffect (prevPhaseRef transition detection), playScream on SSE SHOT_RECEIVED with SUNK, playLaugh/playScream on SSE GAME_OVER, stopSoundtrack on surrender/gameExpired. Modified `client/src/app/settings/page.tsx` (+25) — Sound section with toggle switch (bg-primary when enabled, bg-surface-secondary when muted). Created 21 silent placeholder MP3 files (363 bytes each) at `client/public/audio/default-{laugh,scream,soundtrack}.mp3` and `client/public/avatars/{luffy,zoro,robin,chopper,nami,ace}/{laugh,scream,soundtrack}.mp3`.
